    ...
    <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm p-4 sm:p-7 overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-4 border-b border-slate-100">
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <span className="text-[11px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1 rounded-full bg-slate-50 text-slate-600 border border-slate-200">
            {Math.round(fillPercent)}% Journey Complete
          </span>
        </div>
      </div>

      {/* Horizontal scroll wrapper on mobile with touch-pan */}
      <div className="overflow-x-auto pb-2 pt-6 sm:pt-8 scrollbar-none touch-pan-x -mx-1 px-1">
        <div className="min-w-[480px] sm:min-w-0 px-2 sm:px-6">
          <div className="relative flex items-center justify-between">
            ...
            {levels.map((lvl, i) => {
              ...
              return (
                <div key={lvl} className="flex flex-col items-center relative z-10">
                  <div
                    className={`w-9 h-9 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl flex items-center justify-center font-bold text-xs sm:text-sm transition-all duration-300 shadow-sm ${isPassed
                  ...
                  <div className="flex flex-col items-center mt-2">
                    ...
                    {isCurrent && (
                      <span className="text-[8px] sm:text-[9px] font-extrabold uppercase tracking-wider px-1.5 py-0.2 rounded bg-orange-100 text-orange-700 mt-0.5">
                        Current
                      </span>
                    )}
                    {isUpcoming && (
                      <span className="text-[8px] sm:text-[9px] text-slate-300 mt-0.5">—</span>
                    )}
                  </div>
                </div>
              );
            ...
            <div className="flex flex-col items-center relative z-10">
              <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 text-white flex items-center justify-center shadow-md shadow-amber-200">
                <FaTrophy className="text-xs sm:text-sm" />
              </div>
              <span className="text-[10px] sm:text-[11px] font-bold text-amber-600 mt-2">Goal</span>
              <span className="text-[8px] sm:text-[9px] font-medium text-slate-400 mt-0.5">Placed</span>
            </div>
          </div>
        </div>
      </div>
...
export default function StudentReport() {
  ...
  const softSkillsSection = useMemo(() => {
    const fromRC = getSection("SoftSkillsRating");
    if (fromRC?.hasSyllabusTasks || fromRC?.items?.some(i => i.isFromSyllabus || i.completedTasks !== undefined)) {
      return fromRC;
    }
    if (taskPerformance?.softSkills?.hasSyllabusTasks && taskPerformance.softSkills.items?.length > 0) {
      return taskPerformance.softSkills;
    }
    return fromRC;
  }, [dynamicSections, taskPerformance]);
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-16 print:min-h-0 print:bg-white print:p-0 print:m-0 print:pb-0">
        ...
        >
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <button
              ...
              title="Print Report"
              className="p-2 sm:p-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50 hover:text-slate-900 transition-all shadow-xs"
            >
              <FaPrint size={14} />
            </button>
            ...
            <PDFDownloadLink
              ...
              className="flex items-center gap-1.5 px-2.5 sm:px-3.5 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-xs font-bold transition-all shadow-xs shadow-emerald-200"
            >
              {({ loading }) =>
                loading ? (
                  <span className="flex items-center gap-1">
                    <span className="animate-spin text-xs">⏳</span> <span className="hidden sm:inline">Preparing...</span>
                  </span>
                ) : (
                  <>
                    <FaDownload size={12} />
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 sm:px-3.5 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-xs font-bold transition-all shadow-xs shadow-orange-200"
            >
              <RiEdit2Fill size={14} />
      ...
      <div className="w-full px-3 sm:px-6 lg:px-8 pt-2 sm:pt-4 space-y-3.5 sm:space-y-6 print:max-w-none print:p-0 print:m-0 print:space-y-4 max-w-[1600px] mx-auto">
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div className="p-3.5 sm:p-6 lg:p-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3.5 sm:gap-4 pb-4 sm:pb-6 border-b border-slate-100">
              <div className="flex items-center gap-2.5 sm:gap-4">
                {/* College Logo */}
                <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-white border border-slate-200/80 p-1 sm:p-1.5 flex items-center justify-center shrink-0 shadow-2xs" title="SSISM College Logo">
                  <img src={collegeLogo} alt="SSISM Logo" className="w-full h-full object-contain" />
                </div>

                {/* Department Logo */}
                <div className="h-10 px-2 sm:h-14 sm:px-3 rounded-xl sm:rounded-2xl bg-white border border-slate-200/80 p-1 sm:p-1.5 flex items-center justify-center shrink-0 shadow-2xs" title="Department Logo">
                  <img src={departmentLogo} alt="Department Logo" className="h-full w-auto max-w-[70px] sm:max-w-[110px] object-contain" />
                </div>
            ...
            <div className="mt-4 sm:mt-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-6">
              <div className="flex items-start sm:items-center gap-3.5 sm:gap-5">
                {studentData.image ? (
                  <img
                    src={studentData.image}
                    alt={fullName}
                    className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl object-cover border-2 border-white shadow-md shadow-slate-200 shrink-0"
                  />
                ) : (
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-orange-500 to-amber-500 text-white flex items-center justify-center text-xl sm:text-2xl font-black shadow-md shadow-orange-100 border-2 border-white shrink-0">
                    {initials}
                  </div>
                )}
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 truncate">{fullName}</h2>
                    <span className="text-[10px] sm:text-xs font-bold px-2.5 py-0.5 rounded-full bg-orange-50 text-orange-600 border border-orange-200 shrink-0">
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2 mt-2 flex-wrap">
                    <span className="text-[11px] sm:text-xs font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg bg-slate-50 text-slate-600 border border-slate-100">
              ...
              <div className="bg-slate-50/80 rounded-2xl p-3.5 sm:p-4 border border-slate-100/80 flex flex-col justify-center w-full lg:w-auto lg:min-w-[260px]">
        ...
        {/* ── Executive KPI Metric Cards (2x2 Grid on Mobile) ── */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
          {/* Metric 1: Overall Grade */}
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 p-3.5 sm:p-5 shadow-sm space-y-1 relative overflow-hidden group hover:border-orange-200 transition">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">OVERALL GRADE</p>
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-orange-50 text-orange-500 border border-orange-100 flex items-center justify-center shrink-0">
                <FaTrophy size={13} />
              </div>
            </div>
            <div className="flex items-baseline gap-1.5 sm:gap-2 flex-wrap">
              ...
              <span className={`text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full ${gradeStyle.bg} truncate`}>
                {gradeStyle.label}
              </span>
            </div>
            <p className="text-[11px] sm:text-xs font-semibold text-slate-400 pt-0.5 truncate">
              Performance Index: Outstanding
            </p>
          </div>

          {/* Metric 2: CGPA */}
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 p-3.5 sm:p-5 shadow-sm space-y-1 relative overflow-hidden group hover:border-blue-200 transition">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">CUMULATIVE CGPA</p>
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-blue-50 text-blue-500 border border-blue-100 flex items-center justify-center shrink-0">
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 p-3.5 sm:p-5 shadow-sm space-y-1 relative overflow-hidden group hover:border-emerald-200 transition">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">ATTENDANCE RATE</p>
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-emerald-50 text-emerald-500 border border-emerald-100 flex items-center justify-center shrink-0">
            ...
            <div className="flex items-baseline gap-1.5 sm:gap-2">
              ...
              <span className="text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100">
                Active
              </span>
            </div>
            <p className="text-[11px] sm:text-xs font-semibold text-slate-400 pt-0.5 truncate">
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 p-3.5 sm:p-5 shadow-sm space-y-1 relative overflow-hidden group hover:border-violet-200 transition">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">CAREER READINESS</p>
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-violet-50 text-violet-500 border border-violet-100 flex items-center justify-center shrink-0">
                <FaRocket size={13} />
              </div>
            </div>
            <div className="flex items-baseline gap-1.5 sm:gap-2 flex-wrap">
              <h3 className="text-2xl sm:text-3xl font-black text-slate-900 group-hover:text-violet-600 transition">
                {reportCardData?.careerReadiness?.placementReady || "Ready"}
              </h3>
              <span className="text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full bg-violet-50 text-violet-700 border border-violet-100 truncate">
                Interview Eligible
              </span>
            </div>
            <p className="text-[11px] sm:text-xs font-semibold text-slate-400 pt-0.5 truncate">
        ...
        {/* ── Subject-Wise Performance (Desktop Table + Mobile Responsive Cards) ── */}
            ...
            <div className="block md:hidden p-3.5 space-y-3">
              {subjectPerformanceSection?.items?.length > 0 ? (
                subjectPerformanceSection.items.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="bg-slate-50/60 rounded-2xl p-3.5 border border-slate-100 space-y-2.5">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <span className="font-bold text-slate-800 text-xs sm:text-sm truncate">{item.itemName}</span>
                        </div>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border shrink-0 ${levelColor}`}>
                          {level}
                        </span>
                      </div>
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center justify-between text-[11px]">
                          <span className="text-slate-500 font-medium">Tasks Completion</span>
                          <span className="font-bold text-slate-700">{evaluated} / {total} tasks ({pct}%)</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between pt-1 border-t border-slate-100 text-[11px]">
                        ...
                        <div className="flex items-center gap-1.5">
                          <StarRating rating={rating} size="text-[10px]" />
                ...
                taskPerformance.technicalSkills.map((tech, idx) => (
                  <div key={idx} className="bg-slate-50/60 rounded-2xl p-3.5 border border-slate-100 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-bold text-slate-800 text-xs truncate">{tech.skillName}</span>
                    ...
                    <div className="flex items-center justify-between text-[11px]">
                      ...
                      <span className="font-bold text-slate-700">{tech.completedTasks || 0} / {tech.totalTasks || 0} ({tech.totalPercentage}%)</span>
                    </div>
                  </div>
                ))
              ) : (
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:p-7">
          <div className="flex items-center justify-between pb-4 sm:pb-5 border-b border-slate-100 mb-4 sm:mb-6">
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-blue-50 text-blue-600 border border-blue-100 flex items-center justify-center shadow-xs shrink-0">
                <FaGraduationCap size={17} />
              </div>
              <div>
                <h3 className="text-sm sm:text-base font-bold text-slate-800">Academic SGPA & CGPA Breakdown</h3>
                <p className="text-[11px] sm:text-xs text-slate-400">University semester performance evaluation</p>
              </div>
            </div>
            <span className="text-[11px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 rounded-full bg-blue-50 text-blue-700 border border-blue-200 shrink-0">
              Grading: 10.0
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 sm:gap-4 items-center">
            ...
            <div className="md:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3">
              {sgpaList.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="bg-slate-50/70 border border-slate-100 rounded-2xl p-3.5 sm:p-4 hover:bg-slate-50 transition">
                    <p className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-slate-400">{yearTitle}</p>
                    <div className="flex items-baseline justify-between mt-1.5 sm:mt-2">
                      <span className="text-xl sm:text-2xl font-black text-slate-800">{score}</span>
                      <span className="text-xs font-bold text-slate-400">SGPA</span>
                    </div>
                    <div className="w-full bg-slate-200 rounded-full h-1.5 mt-2.5 sm:mt-3 overflow-hidden">
                      <div
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-2xl p-4 sm:p-6 text-center shadow-md relative overflow-hidden">
              <div className="absolute top-0 right-0 w-24 h-24 bg-orange-500/10 rounded-full blur-xl pointer-events-none" />
              <p className="text-[10px] sm:text-[11px] uppercase font-bold tracking-widest text-slate-400">CUMULATIVE CGPA</p>
              <p className="text-3xl sm:text-4xl font-black text-white mt-1 tracking-tight">{cgpaValue}</p>
              <div className="mt-2 inline-flex items-center gap-1 text-[11px] sm:text-xs font-bold text-orange-400 bg-white/10 px-2.5 py-0.5 rounded-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:p-7 space-y-4 sm:space-y-5">
            <div className="flex items-center justify-between pb-3.5 border-b border-slate-100">
              <div className="flex items-center gap-2.5 sm:gap-3">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-violet-50 text-violet-600 border border-violet-100 flex items-center justify-center shadow-xs shrink-0">
                  <FaBrain size={16} />
                </div>
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-sm sm:text-base font-bold text-slate-800">Soft Skills & Behavioural Assessment</h3>
                    {(softSkillsSection?.hasSyllabusTasks || softSkillsSection?.items?.some(i => i.isFromSyllabus || i.completedTasks !== undefined)) && (
                      <span className="text-[10px] sm:text-xs font-black px-2.5 py-0.5 rounded-full bg-violet-100/70 text-violet-800 border border-violet-200">
                  ...
                  <p className="text-[11px] sm:text-xs text-slate-400">
                    {(softSkillsSection?.hasSyllabusTasks || softSkillsSection?.items?.some(i => i.isFromSyllabus || i.completedTasks !== undefined))
                      ...
                      : "Interpersonal, teamwork and presentation evaluations"}
                  </p>
                </div>
              </div>
              <span className="text-[11px] sm:text-xs font-bold px-2.5 py-1 rounded-full bg-violet-50 text-violet-700 border border-violet-100 shrink-0">
                Max 5.0
              </span>
            </div>
            ...
            <div className="space-y-2.5 sm:space-y-3.5">
              {(softSkillsSection?.items?.length > 0
                ...
                return (
                  <div key={idx} className="bg-slate-50/70 rounded-xl sm:rounded-2xl p-3 sm:p-3.5 border border-slate-100 space-y-1.5">
                    <div className="flex items-center justify-between mb-1 gap-2 flex-wrap sm:flex-nowrap">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-xs font-bold text-slate-700 truncate" title={item.itemName}>
                          {item.itemName}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        {isSyllabus && item.completedTasks !== undefined && (
                          <span className="text-[10px] sm:text-[11px] font-bold text-slate-500 bg-white px-2 py-0.5 rounded border border-slate-200">
                            {item.completedTasks}/{item.totalTasks} Tasks ({pct}%)
                          </span>
                        )}
                        {isSyllabus && item.totalMaxMarks > 0 && (
                        ...
                        {item.remark && (
                          <span className="text-[10px] sm:text-[11px] font-bold text-violet-700 bg-violet-50 px-2 py-0.5 rounded border border-violet-200/80">
                            {item.remark}
                          </span>
                        )}
                        <div className="flex items-center gap-1.5">
                    ...
                    {/* Progress bar according to task completion */}
                    <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden mt-1.5">
                      <div
                        className={`h-2 rounded-full ${isSyllabus ? "bg-gradient-to-r from-violet-600 to-indigo-600" : "bg-violet-600"} transition-all duration-500`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:p-7">
          <div className="flex items-center justify-between pb-4 sm:pb-5 border-b border-slate-100 mb-4 sm:mb-6">
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-emerald-50 text-emerald-600 border border-emerald-100 flex items-center justify-center shadow-xs shrink-0">
                <FaRocket size={16} />
              </div>
              <div>
                <h3 className="text-sm sm:text-base font-bold text-slate-800">Placement & Career Readiness Milestones</h3>
                <p className="text-[11px] sm:text-xs text-slate-400">Industry onboarding and hiring drive preparedness status</p>
              </div>
            </div>
            <span className="text-[11px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 shrink-0">
              T&P Cell
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-4">
            {(careerReadinessSection?.items?.length > 0
              ...
              return (
                <div key={idx} className="bg-slate-50/70 rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-slate-100 text-center hover:bg-slate-50 transition">
                  <p className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 sm:mb-2 truncate">
                    {item.itemName}
                  </p>
                  <span className={`inline-flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full text-[11px] sm:text-xs font-bold border ${badgeColor} max-w-full truncate`}>
                    {isReady ? <FaCheck size={9} /> : <span className="w-1.5 h-1.5 rounded-full bg-current shrink-0" />}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          {/* Discipline Stats */}
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:p-7 space-y-3.5 sm:space-y-4">
            <div className="flex items-center gap-2.5 sm:gap-3 pb-3 border-b border-slate-100">
              ...
              <h4 className="text-xs sm:text-sm font-bold text-slate-800">Attendance & Conduct</h4>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
              {(attendanceDisciplineSection?.items?.length > 0
                ...
                <div key={idx} className="bg-slate-50/70 rounded-xl p-2.5 sm:p-3 border border-slate-100 text-center">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 truncate">{item.itemName}</p>
                  <p className="text-sm sm:text-base font-black text-slate-800 mt-0.5 sm:mt-1 truncate">{item.value}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:p-7 space-y-3.5 sm:space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2.5 sm:gap-3">
                ...
                <h4 className="text-xs sm:text-sm font-bold text-slate-800">Co-Curricular & Certifications</h4>
              </div>
            ...
            {reportCardData?.coCurricular && reportCardData.coCurricular.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
                {reportCardData.coCurricular.map((act, idx) => (
                  <div key={idx} className="bg-slate-50/70 rounded-xl sm:rounded-2xl p-3.5 sm:p-4 border border-slate-100 flex items-start gap-2.5 sm:gap-3">
                    <div className="w-8 h-8 rounded-xl bg-white border border-slate-200 text-slate-700 flex items-center justify-center shrink-0 mt-0.5">
                      {act.category?.toLowerCase().includes("sport") ? <MdSports size={15} /> : <FaProjectDiagram size={13} />}
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-slate-800 truncate">{act.title}</p>
                      <p className="text-[11px] font-medium text-slate-500 mt-0.5 truncate">{act.category} · {act.remark}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          {/* Strengths Card */}
          <div className="bg-emerald-50/40 rounded-2xl sm:rounded-3xl border border-emerald-100 p-4 sm:p-6 lg:p-7 space-y-2.5 sm:space-y-3">
            <div className="flex items-center gap-2.5">
              <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-emerald-100 text-emerald-700 flex items-center justify-center text-sm font-bold shrink-0">
                💪
              </span>
              <h4 className="text-sm sm:text-base font-bold text-emerald-950">Demonstrated Strengths</h4>
            </div>
            <p className="text-[11px] sm:text-xs text-emerald-700/80">Key student traits observed by course faculties</p>
            ...
            <ul className="space-y-2 pt-1 sm:pt-2">
              {(() => {
                ...
                return points.map((pt, idx) => (
                  <li key={idx} className="flex items-start gap-2 text-xs font-semibold text-emerald-900">
          ...
          <div className="bg-amber-50/40 rounded-2xl sm:rounded-3xl border border-amber-100 p-4 sm:p-6 lg:p-7 space-y-2.5 sm:space-y-3">
            <div className="flex items-center gap-2.5">
              <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-amber-100 text-amber-700 flex items-center justify-center text-sm font-bold shrink-0">
                🎯
              </span>
              <h4 className="text-sm sm:text-base font-bold text-amber-950">Areas for Focused Growth</h4>
            </div>
            <p className="text-[11px] sm:text-xs text-amber-700/80">Target development areas prior to final recruitment</p>
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:p-8 relative overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 pb-4 sm:pb-5 border-b border-slate-100 mb-4 sm:mb-5">
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-orange-50 text-orange-500 border border-orange-100 flex items-center justify-center shrink-0">
                <RiDoubleQuotesL size={18} />
              </div>
              <div>
                <h4 className="text-sm sm:text-base font-bold text-slate-800">Faculty Mentorship & Recommendation</h4>
                <p className="text-[11px] sm:text-xs text-slate-400">Formal observation recorded by Academic Evaluator</p>
              </div>
            </div>
          ...
          <div className="bg-slate-50/80 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-slate-100/80 relative">
            ...
            <div className="mt-3.5 sm:mt-4 pt-3.5 sm:pt-4 border-t border-slate-200/60 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs font-semibold text-slate-500">
              <div className="flex items-center gap-2">
